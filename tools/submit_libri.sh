...
job_dir=k8s_job
num_node=1
job_name=librispeech_fast_u2++_conformer_sweight0.5_distill0.0_schedule_ns_slayer7_handns
...
dir=exp/librispeech_fast_u2++_conformer_sweight0.5_distill0.0_schedule_ns_slayer7_handns
hdfs_dir=librispeech_fast_u2++_conformer_sweight0.5_distill0.0_schedule_ns_slayer7_handns
...
for f in $data conf path.sh train_cluster.sh decode.sh wenet tools; do
  cp -L -r $f $job_dir
done
