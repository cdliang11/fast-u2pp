...
def main():
    ...
    for i, (config, ckpt, tag, chunk) in enumerate(
        ...
        with torch.no_grad():
            ...
            for j, encoder_out in enumerate([l_encoder_out, h_encoder_out, lh_encoder_out]):
                if j == 0:
                    hyps, scores = ctc_greedy(encoder_out, encoder_mask, model, batch_size, eos, is_l_ctc=True)
                else:
            ...
            # scores = [prob.tolist() for prob in topk_prob]
                ...
                axes[j].set_title('{} chunk_{}'.format(llist[j], chunk_size_list[i]), fontsize=30)
